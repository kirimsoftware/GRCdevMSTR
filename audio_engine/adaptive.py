...
def match_reference_eq(source_audio, source_sr, ref_audio, ref_sr, strength=0.6):
    """EQ matching presisi: bandingkan kurva spektrum source vs reference
    (keduanya tilt-aware, mean-normalized) -> koreksi EQ agar source
    mendekati tonal balance reference.

    strength: 0..1 (0.6 = natural, 1.0 = agresif). Dibatasi MAX_EQ_CORRECTION."""
    strength = max(0.0, min(1.0, float(strength)))
    src_curve = _measured_curve(source_audio, source_sr)   # tilt-aware, mean 0
    ref_curve = _measured_curve(ref_audio, ref_sr)         # tilt-aware, mean 0
    raw = (ref_curve - src_curve) * strength
    smooth = np.convolve(raw, [0.2, 0.6, 0.2], mode='same')
    eq_gains = {str(f): float(np.clip(g, -MAX_EQ_CORRECTION, MAX_EQ_CORRECTION))
                for f, g in zip(EQ_BANDS, smooth)}
    return {
        'eq_gains': eq_gains,
